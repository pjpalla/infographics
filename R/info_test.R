...
grid.text(paste(
  "Fonte:",
  "Maintainer:",
  "Intervallo temporale:", sep = "\n"), vjust = 0, hjust = 0, x = unit(0.01, "npc"), y = unit(0.79, "npc"), gp = gpar(fontfamily = "Impact", col = "#552683", cex = 0.8))
grid.text(paste(
  "SIRED - Sistema Informativo di Raccolta ed Elaborazione Dati, un sistema federato web gestito dalla Regione Autonoma della Sardegna.",
...
pushViewport(viewport(layout = grid.layout(4, 3)))
grid.rect(gp = gpar(fill = "#E2E2E3", col = "#E2E2E3"))
grid.text("REGIONE", y = unit(1, "npc"), x = unit(0.5, "npc"), vjust = 1, hjust = .5, gp = gpar(fontfamily = "Impact", col = "#A9A8A7", cex = 12, alpha = 0.3))
vplayout <- function(x, y) viewport(layout.pos.row = x, layout.pos.col = y)
grid.text("Il Turismo in Sardegna", y = unit(0.94, "npc"), gp = gpar(fontfamily = "Impact", col = "#E7A922", cex = 6.4))
#grid.text("ANNO 2018", vjust = 0, y = unit(0.92, "npc"), gp = gpar(fontfamily = "Impact", col = "#552683", cex = 0.8))
